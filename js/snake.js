...
$( function() {
    ...
    $( window ).resize( function() {
        ...
        config.layers.bg.draw();
        config.layers.foreground.draw();
        config.layers.loading.draw();
        config.layers.menu.draw();
    });
    ...
    function initializeRemainingShapes() {
        generateTilePrototype();
        generateBoundaryPrototype();
    ...
    function generateTilePrototype() {
        config.shapes.game.tileProto = new Kinetic.Rect({
            x: 0 - config.stage.width(),
            y: 0 - config.stage.height(),
    ...
    function generateBoundaryPrototype() {
        config.shapes.game.boundaryProto = new Kinetic.Path({
            x: 0 - config.stage.width(),
            y: 0 - config.stage.height(),
    ...
    function generateBackgroundTiles() {
        ...
        for ( var x = 0; x < config.tiles.horizontalAmount; x++ ){
            for ( var y = 0; y < config.tiles.verticalAmount; y++ ){
                tile = config.shapes.game.tileProto.clone({
                ...
                if ( x == 0 || x == config.tiles.horizontalAmount - 1 ||
                    ...
                    tile = config.shapes.game.boundaryProto.clone({
